...
class Zefoy:
    def __init__(self):
        ...
        self.proxy = {'http': '38.162.30.166:3128'}  # Add your proxy URL here
        self.headers = {
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36'
        }
        self.session4 = requests.Session()  # Custom session variable name
        self.session4.proxies.update(self.proxy)  # Update session with proxy
        self.captcha_1 = None
        self.captcha_ = {}
        self.service = 'Views'
        self.video_key = None
    ...
    def get_captcha(self):
        if os.path.exists('session'):
            self.session4.cookies.set("PHPSESSID", open('session', encoding='utf-8').read(), domain='zefoy.com')
        request = self.session4.get(self.base_url, headers=self.headers)
        ...
        try:
            ...
            request = self.session4.get(f"{self.base_url}{captcha_url}", headers=self.headers)
    ...
    def send_captcha(self, new_session=False):
        ...
        request = self.session4.post(self.base_url, headers=self.headers, data=self.captcha_)

        if 'Enter Video URL' in request.text:
            print('Session was created')
            open('session', 'w', encoding='utf-8').write(self.session4.cookies.get('PHPSESSID'))
            self.video_key = request.text.split('" placeholder="Enter Video URL"')[0].split('name="')[-1]
    ...
    def solve_captcha(self, path_to_file=None, b64=None, delete_tag=['\n', '\r']):
        ...
        request = self.session4.post('https://api.ocr.space/parse/image?K87899142388957', headers={'apikey': 'K87899142388957'}, files={'task': open(task, 'rb')}).json()
    ...
    def get_status_services(self):
        request = self.session4.get(self.base_url, headers=self.headers).text
    ...
    def find_video(self):
        ...
        while True:
            if self.service not in self.services_ids:
                ...
                time.sleep(1)
            request = self.session4.post(f'{self.base_url}{self.services_ids[self.service]}', headers={'content-type': 'multipart/form-data; boundary=----WebKitFormBoundary0nU8PjANC8BhQgjZ', 'user-agent': self.headers['user-agent'], 'origin': 'https://zefoy.com'}, data=f'------WebKitFormBoundary0nU8PjANC8BhQgjZ\r\nContent-Disposition: form-data; name="{self.video_key}"\r\n\r\n{self.url}\r\n------WebKitFormBoundary0nU8PjANC8BhQgjZ--\r\n')
            try:
    ...
    def use_service(self):
        ...
        request = self.session4.post(f'{self.base_url}{self.services_ids[self.service]}', headers={'content-type': f'multipart/form-data; boundary=----WebKitFormBoundary{self.token}', 'user-agent': self.headers['user-agent'], 'origin': 'https://zefoy.com'}, data=f'------WebKitFormBoundary{self.token}\r\nContent-Disposition: form-data; name="{self.video_info[0]}"\r\n\r\n{self.video_info[1]}\r\n------WebKitFormBoundary{self.token}--\r\n')
        try:
    ...
    def get_video_info(self):
        request = self.session4.get(f'https://tiktok.livecounts.io/video/stats/{urlparse(self.url).path.rpartition("/")[2]}', headers={'authority': 'tiktok.livecounts.io', 'origin': 'https://livecounts.io', 'user-agent': self.headers['user-agent']}).json()
